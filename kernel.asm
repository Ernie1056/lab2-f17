...
80100bff:	e8 7c 60 00 00       	call   80106c80 <copyout>
...
80100c20:	e8 4b 1f 00 00       	call   80102b70 <end_op>
    cprintf("exec: fail\n");
80100c25:	c7 04 24 21 6e 10 80 	movl   $0x80106e21,(%esp)
...
80100c7d:	89 95 60 ff ff ff    	mov    %edx,-0xa0(%ebp)
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100c83:	e8 f8 5f 00 00       	call   80106c80 <copyout>
...
801044c3:	8b 45 08             	mov    0x8(%ebp),%eax
  ...
  if(addr >= USERSTACKSTART)
801044c6:	3d fe ff ff 7f       	cmp    $0x7ffffffe,%eax
...
80104554:	8d 44 90 04          	lea    0x4(%eax,%edx,4),%eax
  if(addr >= USERSTACKSTART)
80104558:	3d fe ff ff 7f       	cmp    $0x7ffffffe,%eax
...
8010459e:	78 20                	js     801045c0 <argptr+0x40>
    return -1;
  if(size < 0 || (uint)i >= USERSTACKSTART || (uint)i+size > USERSTACKSTART)
...
80106aa3:	0f 84 78 01 00 00    	je     80106c21 <copyuvm+0x191>
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
...
80106b17:	0f 84 17 01 00 00    	je     80106c34 <copyuvm+0x1a4>
    if(!(*pte & PTE_P))
80106b1d:	8b 30                	mov    (%eax),%esi
80106b1f:	f7 c6 01 00 00 00    	test   $0x1,%esi
80106b25:	0f 84 fd 00 00 00    	je     80106c28 <copyuvm+0x198>
    pa = PTE_ADDR(*pte);
...
80106b5e:	5d                   	pop    %ebp
80106b5f:	c3                   	ret    
    for(i = USERSTACKSTART - PGSIZE + 1; stackSize > 0; i -= PGSIZE, stackSize--)
...
80106ba5:	8d 87 00 00 00 80    	lea    -0x80000000(%edi),%eax
80106bab:	89 44 24 0c          	mov    %eax,0xc(%esp)
...
80106bbc:	78 56                	js     80106c14 <copyuvm+0x184>
    for(i = USERSTACKSTART - PGSIZE + 1; stackSize > 0; i -= PGSIZE, stackSize--)
...
80106bd8:	74 5a                	je     80106c34 <copyuvm+0x1a4>
        if(!(*pte & PTE_P))
80106bda:	8b 38                	mov    (%eax),%edi
80106bdc:	f7 c7 01 00 00 00    	test   $0x1,%edi
80106be2:	74 44                	je     80106c28 <copyuvm+0x198>
        pa = PTE_ADDR(*pte);
80106be4:	89 fe                	mov    %edi,%esi
        flags = PTE_FLAGS(*pte);
80106be6:	81 e7 ff 0f 00 00    	and    $0xfff,%edi
80106bec:	89 7d e4             	mov    %edi,-0x1c(%ebp)
        pa = PTE_ADDR(*pte);
...
80106c13:	c3                   	ret    
            kfree(mem);
80106c14:	89 3c 24             	mov    %edi,(%esp)
80106c17:	e8 c4 b6 ff ff       	call   801022e0 <kfree>
            goto bad;
80106c1c:	e9 2a ff ff ff       	jmp    80106b4b <copyuvm+0xbb>
    return 0;
80106c21:	31 c0                	xor    %eax,%eax
80106c23:	e9 30 ff ff ff       	jmp    80106b58 <copyuvm+0xc8>
      panic("copyuvm: page not present");
80106c28:	c7 04 24 56 77 10 80 	movl   $0x80107756,(%esp)
80106c2f:	e8 2c 97 ff ff       	call   80100360 <panic>
      panic("copyuvm: pte should exist");
80106c34:	c7 04 24 3c 77 10 80 	movl   $0x8010773c,(%esp)
80106c3b:	e8 20 97 ff ff       	call   80100360 <panic>
